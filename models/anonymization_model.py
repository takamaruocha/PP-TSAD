import torch
import torch.nn as nn
import torch.nn.functional as F
from einops import rearrange, repeat

from .encoder import Encoder
from .decoder import Decoder
from .attn import FullAttention, AttentionLayer, TwoStageAttentionLayer
from .embed import DSW_embedding

from math import ceil


class AnonymizationModel(nn.Module):
    def __init__(self, data_dim, in_len, seg_len, n_classes, win_size=4,
                factor=10, d_model=512, d_ff=1024, n_heads=8, e_layers=3, 
                dropout=0.0, baseline=False, device=torch.device('cuda:0')):
        super(AnonymizationModel, self).__init__()
        self.data_dim = data_dim
        self.in_len = in_len
        self.seg_len = seg_len
        #self.merge_win = win_size

        self.baseline = baseline

        self.device = device

        # The padding operation to handle invisible segment length
        self.pad_in_len = ceil(1.0 * in_len / seg_len) * seg_len
        self.in_len_add = self.pad_in_len - self.in_len

        # Embedding sensitive information
        self.embedding_sensitive_info = nn.Parameter(torch.zeros(n_classes, 1, data_dim))
        #self.embedding_sensitive_info = nn.Parameter(torch.randn(n_classes, 1, data_dim))

        # Embedding
        self.enc_value_embedding = DSW_embedding(seg_len, d_model) # H which aggregates h_(i, d) in eq.(2)
        self.enc_pos_embedding = nn.Parameter(torch.randn(1, data_dim, (self.pad_in_len // seg_len), d_model))
        self.pre_norm = nn.LayerNorm(d_model)

        # Encoder
        self.encoder = Encoder(e_layers, win_size, d_model, n_heads, d_ff, block_depth = 1, \
                                    dropout = dropout, in_seg_num = (self.pad_in_len // seg_len), factor = factor)
        
        # Decoder
        self.dec_pos_embedding = nn.Parameter(torch.randn(1, data_dim, (self.pad_in_len // seg_len), d_model))
        self.decoder = Decoder(seg_len, e_layers + 1, d_model, n_heads, d_ff, dropout, \
                                    out_seg_num = (self.pad_in_len // seg_len), factor = factor)
        
    def forward(self, x): # x_seq: (batch_size, in_len, data_dim) 
        if (self.baseline):
            base = x.mean(dim = 1, keepdim = True)
        else:
            base = 0
        batch_size = x.shape[0]

        if (self.in_len_add != 0): # padding operation
            x = torch.cat((x_seq[:, :1, :].expand(-1, self.in_len_add, -1), x), dim = 1)

        x_seq = self.enc_value_embedding(x) # (batch_size, in_len, data_dim) -> (batch_size, data_dim, seg_num, d_model)
        x_seq += self.enc_pos_embedding
        x_seq = self.pre_norm(x_seq)
        
        enc_out = self.encoder(x_seq)

        dec_in = repeat(self.dec_pos_embedding, 'b ts_d l d -> (repeat b) ts_d l d', repeat = batch_size)
        predict_y = self.decoder(dec_in, enc_out)


        return base + predict_y[:, :self.in_len, :]
